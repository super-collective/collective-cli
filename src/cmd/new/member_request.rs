...
pub struct NewMemberRequestCommand {
	#[clap(index = 1, default_value = "cli")]
	mode: GenerationMode,

	/// The evidence folder.
	#[clap(long, default_value = "member_requests")]
	requests: PathBuf,
...
impl NewMemberRequestCommand {
	pub fn run(&self) -> Result<()> {
		let data = match self.mode {
			GenerationMode::Template => MemberRequest::template().into(),
